...
const APPWRITE_FUNCTION_EVENT_DATA = JSON.parse(
  process.env.APPWRITE_FUNCTION_EVENT_DATA
);

if (APPWRITE_FUNCTION_EVENT_DATA && APPWRITE_FUNCTION_EVENT_DATA.$id) {
  const { $id: fileID } = APPWRITE_FUNCTION_EVENT_DATA;
  ...
  const cloudmersiveDefaultClient =
    CloudmersiveImageApiClient.ApiClient.instance;
  cloudmersiveDefaultClient.authentications.Apikey.apiKey =
    CLOUDMERSIVE_API_KEY;

  const cloudmersiveRecognizeApi =
    new CloudmersiveImageApiClient.RecognizeApi();
